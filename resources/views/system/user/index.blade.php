...
@section('content')
    <div class="bg-white p-6 rounded-lg shadow mb-6">
        <h2 class="text-2xl font-bold mb-4">List Anggota Verifikasi</h2>
        <div class="flex justify-between items-center mb-4">
            @if (!empty($data['PermissionAdd']))
                <a href="{{ route('users.create') }}" class="px-4 py-2 bg-pink-500 text-white rounded shadow">Tambah</a>
            @endif
        </div>
            ...
            <table class="w-full border border-gray-300">
                <thead>
                    <tr class="bg-gray-100">
                        ...
                        <th class="px-4 py-2 border-b border-gray-300 text-left">FullNama</th>
                        <th class="px-4 py-2 border-b border-gray-300 text-left">Username</th>
                ...
                <tbody>
                    @foreach ($data['user'] as $row)
                        <tr>
                            ...
                            <td class="px-4 py-2 border-b border-gray-300 text-left">{{ $row->fullname }}</td>
                            <td class="px-4 py-2 border-b border-gray-300 text-left">{{ $row->username }}</td>
                            ...
                            <td class="flex py-2 space-x-1 items-center">
                                ...
                                @if (!empty($data['PermissionDelete']))
                                    <button type="button"
                                        ...
                                        onclick="confirmDelete('{{ route('users.destroy', $row->id) }}', '{{ $row->fullname }}')">
